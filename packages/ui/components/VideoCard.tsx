    ...
    <Link href={`/${type}/${video._id}`}>
      <div className="bg-white rounded-xl shadow-lg cursor-pointer hover:shadow-2xl transition-all duration-300 overflow-hidden">
        ...
        <div className="p-4">
          ...
          {<p>Uploader: {(video.creator.name).charAt(0).toUpperCase() + video.creator.name.slice(1)}</p>}
          {<p>Editor: {(video.editor?.name)?.charAt(0).toUpperCase() + video.editor?.name?.slice(1) || 'Nill'}</p>}
