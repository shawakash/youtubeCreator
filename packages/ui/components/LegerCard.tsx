...
export const LegerCard: React.FC<{ leger: legerType, clientId: string }> = ({ leger, clientId }) => {
    ...
    return (
        <>
                <div className="bg-white w-[500px] shadow-lg  gap-y-2 flex flex-col hover:shadow-2xl transition-all rounded-2xl">
                        ...
                        <div className="relative">
                            <div className="aspect-w-16 aspect-h-9">
                                <video className="w-[500px]" controls>
                                    <source src={leger.rawVideo.url} type={leger.rawVideo.contentType} />
                                    Your browser does not support the video tag.
                                </video>
                            </div>
                        </div>
                    </div>
            <Link href={`/videos/raw/${leger.rawVideo._id}`}>
                    <div className="px-4">
                    ...
                    <div className='px-4 mb-4'>
                        <p className={hasEditors ? 'text-green-600' : 'text-red-600'}>
                            Editor: {hasEditors ? clientId == leger.rawVideo.editor._id ? 'You' : 'Editor Assigned' : 'None'}
                        </p>
                    </div>
                </div>
        </>
